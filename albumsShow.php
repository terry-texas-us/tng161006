    ...
    ?>
      <table class='table table-sm'>
        ...
        while ($row = tng_fetch_assoc($result)) {
          ...
          $query = "SELECT $album2entities_table.entityID AS personID, people.personID AS personID2, people.living AS living, people.private AS private, people.branch AS branch, $families_table.branch AS fbranch, $families_table.living AS fliving, $families_table.private AS fprivate, familyID, husband, wife, people.lastname AS lastname, people.lnprefix AS lnprefix, people.firstname AS firstname, people.prefix AS prefix, people.suffix AS suffix, nameorder, sources.title, sources.sourceID, $repositories_table.repoID, reponame, deathdate, burialdate, linktype FROM $album2entities_table LEFT JOIN $people_table AS people ON $album2entities_table.entityID = people.personID LEFT JOIN $families_table ON $album2entities_table.entityID = $families_table.familyID LEFT JOIN sources ON $album2entities_table.entityID = sources.sourceID LEFT JOIN $repositories_table ON ($album2entities_table.entityID = $repositories_table.repoID) WHERE albumID = '{$row['albumID']}' ORDER BY lastname, lnprefix, firstname, personID LIMIT $maxplus";
