            ...
            ?>
              <table class="table table-sm table-striped">
                ...
                while ($row = tng_fetch_assoc($result)) {
                  ...
                  $query = "SELECT people.personID AS personID2, familyID, husband, wife, people.lastname AS lastname, people.lnprefix AS lnprefix, people.firstname AS firstname, people.prefix AS prefix, people.suffix AS suffix, nameorder, $album2entities_table.entityID AS personID, $sources_table.title, $sources_table.sourceID, $repositories_table.repoID, reponame FROM $album2entities_table LEFT JOIN $people_table AS people ON $album2entities_table.entityID = people.personID LEFT JOIN $families_table ON $album2entities_table.entityID = $families_table.familyID LEFT JOIN $sources_table ON $album2entities_table.entityID = $sources_table.sourceID LEFT JOIN $repositories_table ON ($album2entities_table.entityID = $repositories_table.repoID) WHERE albumID = '{$row['albumID']}' ORDER BY lastname, lnprefix, firstname, personID LIMIT 10";
