...
$m = $mostwanted_table;
if ($ID) {
  $query = "SELECT $m.title AS title, $m.personID AS personID, $m.description AS description, $m.mediaID AS mediaID, mwtype, thumbpath, usecollfolder, $media_table.description AS mtitle, $media_table.notes AS mdesc, mediatypeID FROM $m LEFT JOIN $media_table ON $m.mediaID = $media_table.mediaID LEFT JOIN $people_table ON $m.personID = $people_table.personID WHERE $m.ID = '$ID'";
