<?php
/**
 * Name history: ajx_editfamily.php
 */
require 'begin.php';
          ...
          <td>
            ...
            <div id="spouses">
              <table class='table table-sm'>
                <?php
                if ($row['husband']) {
                  $query = "SELECT lastname, lnprefix, firstname, prefix, suffix, nameorder, living, private, branch, birthdate, altbirthdate FROM people WHERE personID = \"{$row['husband']}\"";
                  $spouseresult = tng_query($query);
                  $spouserow = tng_fetch_assoc($spouseresult);
                  tng_free_result($spouseresult);
                }
                if ($row['husband']) {
                  $hrights = determineLivingPrivateRights($spouserow);
                ...
                <?php
                if ($row['wife']) {
                  $query = "SELECT lastname, lnprefix, firstname, prefix, suffix, nameorder, living, private, branch, birthdate, altbirthdate FROM people WHERE personID = \"{$row['wife']}\"";
                  $spouseresult = tng_query($query);
                  $spouserow = tng_fetch_assoc($spouseresult);
                  tng_free_result($spouseresult);
                } else {
                ...
                if ($row['wife']) {
                  $wrights = determineLivingPrivateRights($spouserow);
...
<script>
  ...
  $('#find-children').on('click', function () {
      return findItem('I', 'children', null);
  });
