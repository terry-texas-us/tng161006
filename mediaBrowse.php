      ...
      <form action="admin_updateselectedmedia.php" method='post' name="form2">
        ...
        <table class='table table-sm'>
          ...
          while ($row = tng_fetch_assoc($result)) {
            ...
            $query = "SELECT people.personID AS personID2, familyID, husband, wife, people.lastname AS lastname, people.lnprefix AS lnprefix, people.firstname AS firstname, people.prefix AS prefix, people.suffix AS suffix, nameorder, $medialinks_table.personID AS personID, sources.title, sources.sourceID, $repositories_table.repoID, reponame, linktype FROM $medialinks_table LEFT JOIN $people_table AS people ON $medialinks_table.personID = people.personID LEFT JOIN $families_table ON $medialinks_table.personID = $families_table.familyID LEFT JOIN sources ON $medialinks_table.personID = sources.sourceID LEFT JOIN $repositories_table ON ($medialinks_table.personID = $repositories_table.repoID) WHERE mediaID = '{$row['mediaID']}' ORDER BY lastname, lnprefix, firstname, personID LIMIT 10";
