...
class UploadHandler {
  ...
  function __construct($options = null, $initialize = true, $error_messages = null) {
      ...
      $this->options = [
          ...
          'redirect_allow_target' => '/^' . preg_quote(parse_url($this->getServerVar('HTTP_REFERER'), PHP_URL_SCHEME) . '://' . parse_url($this->getServerVar('HTTP_REFERER'), PHP_URL_HOST) . '/', '/') . '/',
  ...
  protected function validate($uploaded_file, $file, $error, $index) {
    ...
    if (is_int($this->options['max_number_of_files']) && ($this->countFileObjects() >= $this->options['max_number_of_files']) && !is_file($this->getUploadPath($file->name))) {
  ...
  protected function getFileName($file_path, $name, $size, $type, $error, $index, $content_range) {
    $name = $this->trimFileName($file_path, $name, $size, $type, $error, $index, $content_range);
    return $this->getUniqueFilename(
      $file_path,
      $this->fixFileExtension($file_path, $name, $size, $type, $error, $index, $content_range),
      $size,
      $type,
      $error,
      $index,
  ...
  protected function imagickCreateScaledImage($file_name, $version, $options) {
    list($file_path, $new_file_path) = $this->getScaledImageFilePaths($file_name, $version);
    $image = $this->imagickGetImageObject($file_path, !empty($options['crop']) || !empty($options['no_cache']));
  ...
  public function get($print_response = true) {
    ...
    if ($file_name) {
      $response = [
          $this->getSingularParamName() => $this->get_file_object($file_name)
      ];
    } else {
      $response = [
    ...
    if ($upload) {
      if (is_array($upload['tmp_name'])) {
        ...
        foreach ($upload['tmp_name'] as $index => $value) {
          $files[] = $this->handleFileUpload($upload['tmp_name'][$index], $file_name ? $file_name : $upload['name'][$index], $size ? $size : $upload['size'][$index], $upload['type'][$index], $upload['error'][$index], $index, $content_range);
        }
      } else {
        // param_name is a single object identifier like "file",
        // $upload is a one-dimensional array:
        $files[] = $this->handleFileUpload(isset($upload['tmp_name']) ? $upload['tmp_name'] : null, $file_name ? $file_name : (isset($upload['name']) ? $upload['name'] : null), $size ? $size : (isset($upload['size']) ? $upload['size'] : $this->getServerVar('CONTENT_LENGTH')), isset($upload['type']) ? $upload['type'] : $this->getServerVar('CONTENT_TYPE'), isset($upload['error']) ? $upload['error'] : null, null, $content_range);
      }
    }
