...
function getOtherEvents($row) {
  ...
  if ($pedigree['regnotes'] && $row['allow_living'] && $row['allow_private']) {
    $query = "SELECT display, eventdate, eventdatetr, eventplace, age, agency, cause, addressID, info, tag, description, eventID FROM (events, eventtypes) WHERE persfamID = \"{$row['personID']}\" AND events.eventtypeID = eventtypes.eventtypeID AND keep = \"1\" AND parenttag = \"\" ORDER BY eventdatetr, ordernum, tag, description, info, eventID";
