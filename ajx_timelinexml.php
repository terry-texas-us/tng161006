...
foreach ($timeline as $timeentry) {
  ...
  if (count($timeline) == 1 && $rights['both']) {
    $query = "SELECT display, eventdate, eventdatetr, eventplace, info FROM (events, eventtypes)
      WHERE persfamID = \"$timeperson\" AND events.eventtypeID = eventtypes.eventtypeID AND keep = \"1\" AND parenttag = \"\"
