...
function getIndividualRecord($personID, $prevlevel) {
    ...
    if ($lineinfo['level'] == $prevlevel) {
      ...
      switch ($tag) {
        ...
        case 'FAMC':
          ...
          $query = "INSERT IGNORE INTO children (familyID, personID, mrel, frel, parentorder) VALUES('$famc', '$personID', '$relationship', '$relationship', '$parentorder')";
          ...
          if ($relationship) {
            if (!$success) {
              $query = "UPDATE children SET mrel = \"$relationship\", frel = \"$relationship\" WHERE familyID = \"$famc\" AND personID = '$personID'";
              $result = tng_query($query) or die(uiTextSnippet('cannotexecutequery') . ": $query");
            }
            $lineinfo = getLine();
          }
  ...
  if (!$success && $savestate['del'] != 'no') {
    ...
    if ($savestate['neweronly'] && $inschangedt) {
      $query = "SELECT changedate FROM people WHERE personID = '$personID'";
      $result = tng_query($query);
    ...
    if ($goahead) {
      ...
      $query = "UPDATE people SET firstname=\"{$info['GIVN']}\", lnprefix=\"{$info['lnprefix']}\", lastname=\"{$info['SURN']}\"" . $livingstrupd . ", nickname=\"{$info['NICK']}\", prefix=\"{$info['NPFX']}\", suffix=\"{$info['NSFX']}\", title=\"{$info['TITL']}\", birthdate=\"" . $info['BIRT']['DATE'] . '", birthdatetr="' . $info['BIRT']['DATETR'] . '", birthplace="' . $info['BIRT']['PLAC'] . "\", sex=\"{$info['SEX']}\", altbirthdate=\"" . $info['CHR']['DATE'] . '", altbirthdatetr="' . $info['CHR']['DATETR'] . '", altbirthplace="' . $info['CHR']['PLAC'] . '", deathdate="' . $info['DEAT']['DATE'] . '", deathdatetr="' . $info['DEAT']['DATETR'] . '", deathplace="' . $info['DEAT']['PLAC'] . '", burialdate="' . $info['BURI']['DATE'] . '", burialdatetr="' . $info['BURI']['DATETR'] . '", burialplace="' . $info['BURI']['PLAC'] . '", baptdate="' . $info['BAPL']['DATE'] . '", baptdatetr="' . $info['BAPL']['DATETR'] . "\", baptplace=\"$baplplace\", confdate=\"" . $info['CONL']['DATE'] . '", confdatetr="' . $info['CONL']['DATETR'] . "\", confplace=\"$confplace\", initdate=\"" . $info['INIT']['DATE'] . '", initdatetr="' . $info['INIT']['DATETR'] . "\", initplace=\"$initplace\", endldate=\"" . $info['ENDL']['DATE'] . '", endldatetr="' . $info['ENDL']['DATETR'] . "\", endlplace=\"$endlplace\", changedby=\"$currentuser\" $chdatestr$branchstr";
      if ($prifamily) {
