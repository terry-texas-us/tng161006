...
function uiTextSnippet($name, $extra = null) {
  ...
  if (!isset($name) || $name === '') {
    ...
    $query = "SELECT value FROM snippets JOIN languages ON snippets.languageID = languages.languageID "
         . "WHERE snippets.name = '$name' AND languages.folder = '$language';";
    ...
    if ($value === '') {
      if (preg_match("/^[iIfF][0-9]+$/", $name)) { // [ts] Individual (i###) and Family (f###) ids are passed to this function. Modify callers to always call with values expected to be snippets?
