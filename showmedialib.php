  ...
  if ($showlinks) {
    ...
    if ($albumlinkID) {
      ...
      if ($linktype == 'F') {
        ...
        $pagenav .= "<a href=\"repositoriesShowItem.php?repoID=$personID&amp;tree=$tree\">&raquo; " . uiTextSnippet('repository') . "</a>  &nbsp;&nbsp;&nbsp;";
      } elseif ($linktype == 'L') {
        $treestr = $tngconfig['places1tree'] ? "" : "&amp;tree=$tree";
        $pagenav .= "<a href=\"placesearch.php?psearch=$personID$treestr\">&raquo; " . uiTextSnippet('place') . ": $personID</a>  &nbsp;&nbsp;&nbsp;";
      }
    }
  }
...
function getMediaLinkText($mediaID, $ioffset) {
  ...
  $query = "SELECT $medialinks_table.medialinkID, $medialinks_table.personID as personID, people.living as living, people.private as private, people.branch as branch, $medialinks_table.eventID,
