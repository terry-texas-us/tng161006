...
function getNotes($persfamID, $flag) {
  ...
  if ($flag == 'I') {
    $precusttitles = array("BIRT" => uiTextSnippet('born'), "CHR" => uiTextSnippet('christened'), "NAME" => uiTextSnippet('name'), "TITL" => uiTextSnippet('title'), "NPFX" => uiTextSnippet('prefix'), "NSFX" => uiTextSnippet('suffix'), "NICK" => uiTextSnippet('nickname'), "BAPL" => uiTextSnippet('baptizedlds'), "CONL" => uiTextSnippet('conflds'), "INIT" => uiTextSnippet('initlds'), "ENDL" => uiTextSnippet('endowedlds'));
    ...
    $precusttitles = array("ABBR" => uiTextSnippet('shorttitle'), "CALN" => uiTextSnippet('callnum'), "AUTH" => uiTextSnippet('author'), "PUBL" => uiTextSnippet('publisher'), "TITL" => uiTextSnippet('title'));
    $postcusttitles = array();
  }
...
function showEvent($data) {
  ...
  if ($dateplace) {
    ...
    if ($data['place']) {
      ...
      if (!isset($data['np'])) {
        $treestr = $tngconfig['places1tree'] ? "" : "&amp;tree=$tree";
        $output .= " <a href=\"placesearch.php?psearch=" . urlencode($data['place']) . $treestr . "\" title=\"" . uiTextSnippet('findplaces') . "\">\n";
  ...
  if ($output) {
    $editicon = $tentative_edit && $data['event'] && $data['event'] != "NAME" ? "<img class='icon-sm' src='svg/new-message.svg' alt=\"" . uiTextSnippet('editevent') . "\" title=\"" . uiTextSnippet('editevent') . "\" onclick=\"tnglitbox = new ModalDialog('ajx_tentedit.php?tree=$tree&amp;persfamID={$data['entity']}&amp;type={$data['type']}&amp;event={$data['event']}&amp;title={$data['text']}');\" class=\"fakelink\">" : "";
...
function doCustomEvents($entityID, $type, $nomap = 0) {
  ...
  $query = "SELECT display, eventdate, eventdatetr, eventplace, age, agency, cause, addressID, info, tag, description, eventID, collapse FROM "
          . "($events_table, $eventtypes_table) WHERE persfamID = \"$entityID\" AND $events_table.eventtypeID = $eventtypes_table.eventtypeID AND gedcom = \"$tree\" AND keep = \"1\" AND parenttag = \"\" ORDER BY ordernum, tag, description, eventdatetr, info, eventID";
  $custevents = tng_query($query);
...
function getMedia($entity, $linktype) {
  ...
  while ($medialink = tng_fetch_assoc($medialinks)) {
    ...
    if (!$medialink['allow_living'] && ($nonames || $tngconfig['nnpriv'])) {
      ...
      $thismedia['description'] = truncateIt(getXrefNotes(($medialink['altnotes'] ? $medialink['altnotes'] : $medialink['notes']), $tree), $tngconfig['maxnoteprev']);
