...
$dy = $pdf->GetFontSize($rptFont, $rptFontSize) + 0.01;    // line height (note: try to have line height so a specific number of whole lines fits on page
  ...
  if ($result) {
    while ($row = tng_fetch_assoc($result)) {
      ...
      if ($row[$spouse]) {
        ...
        $infoDescend[$j]['name'] = uiTextSnippet('unknown');
      }
