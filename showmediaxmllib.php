...
if ($medialinkID) {
  ...
  if ($albumlinkID) {
    $query = "SELECT albumname, description, ordernum, albums.albumID AS albumID FROM (albums, albumlinks) WHERE albumlinkID = '$albumlinkID' AND albumlinks.albumID = albums.albumID";
    $result = tng_query($query);
    $row = tng_fetch_assoc($result);
    $ordernum = $row['ordernum'];
    $albumID = $row['albumID'];
